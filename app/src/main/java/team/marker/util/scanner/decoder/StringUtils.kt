...
object StringUtils {
     ...
     */
    fun guessEncoding(bytes: ByteArray, hints: Map<ScannerDecodeHintType?, *>?): String {
        if (hints != null && hints.containsKey(ScannerDecodeHintType.CHARACTER_SET)) {
            return hints[ScannerDecodeHintType.CHARACTER_SET].toString()
        }
        ...
        while (i < length && (canBeISO88591 || canBeShiftJIS || canBeUTF8)) {
            val value: Int = bytes[i] and 0xFF
