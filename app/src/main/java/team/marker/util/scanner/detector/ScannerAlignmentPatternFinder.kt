 ...
 * @author Sean Owen
 */
internal class ScannerAlignmentPatternFinder(
    ...
    height: Int,
    moduleSize: Float,
    resultPointCallback: ScannerResultPointCallback?
) {
    private val possibleCenters: MutableList<ScannerAlignmentPattern>
    ...
    fun find(): ScannerAlignmentPattern {
        val startX = startX
        val height = height
     ...
     */
    private fun handlePossibleCenter(stateCount: IntArray, i: Int, j: Int): ScannerAlignmentPattern? {
        val stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2]
        val centerJ = centerFromEnd(stateCount, j)
        val centerI = crossCheckVertical(
            i,
            centerJ.toInt(), 2 * stateCount[1], stateCountTotal
        )
        if (!java.lang.Float.isNaN(centerI)) {
            ...
            val point = ScannerAlignmentPattern(centerJ, centerI, estimatedModuleSize)
